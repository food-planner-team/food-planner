<template>
    <div class="rounded-lg flex flex-col items-center gap-5 p-5 shadow-md bg-[#fefffe] w-[240px] max-h-[330px] mb-2">
        <div class="w-[120px] h-[120px] rounded-md ml-1 flex items-center">
            <img v-if="props.product?.image?.url" :src="`${props.product?.image?.url}`" :alt="props.product.name"
                class="rounded-md" />
            <img v-else src="../../common/assets/no-image.jpg" alt="no image placeholder" class="rounded-md" />
        </div>
        <div class="flex flex-col justify-between flex-1 gap-2 p-1 h-full">
            <div class="flex justify-between">
                ...
                <Switch v-model="props.product.optional" :class="!props.product.optional
                    ? 'bg-primary-dark'
                    : 'bg-gray-200'
                    " class="relative inline-flex h-6 w-11 items-center rounded-full min-w-[43px]"
                    data-te-toggle="tooltip" title="Czy produkt jest wymagany w przepisie?">
                    <span class="sr-only">Enable notifications</span>
                    <span :class="!props.product.optional
                        ? 'translate-x-6'
                        : 'translate-x-1'
